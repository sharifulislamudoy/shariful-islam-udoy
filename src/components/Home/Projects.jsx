...
import { ExternalLink, Github, Calendar, Code, ArrowRight, Eye, RefreshCw } from 'lucide-react';
...
const Projects = () => {
    ...
    const cardVariants = {
        ...
        hover: { y: -10, scale: 1.02, transition: { duration: 0.3, ease: "easeInOut" } }
    };
            ...
            <div className="w-11/12 mx-auto">
                <motion.div variants={itemVariants} className="text-center mb-16">
                    <motion.h2 initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.2 }}
                        ...
                        <span className="bg-gradient-to-r from-blue-400 to-cyan-400 bg-clip-text text-transparent">My Projects</span>
                    </motion.h2>
                    <motion.div initial={{ opacity: 0, width: 0 }} animate={inView ? { opacity: 1, width: 100 } : {}} transition={{ duration: 0.8, delay: 0.5 }}
                        className="h-1 bg-gradient-to-r from-blue-500 to-cyan-500 mx-auto rounded-full w-24" />
                    <motion.p variants={itemVariants} className="text-xl text-gray-300 mt-6 max-w-2xl mx-auto">
                ...
                <div className="relative">
                    ...
                    <div className="h-[700px] overflow-y-auto pr-4 custom-scrollbar relative">
                        <motion.div variants={containerVariants} className="grid grid-cols-1 lg:grid-cols-2 gap-8 pb-4">
                            {projects.length === 0 ? (
                                ...
                                <motion.div key={project._id || index} variants={cardVariants} whileHover="hover" className="group">
                                        ...
                                        <div className="p-6 flex-1 flex flex-col">
                                            <div className="flex-1">
                                                ...
                                                <div className="flex flex-wrap gap-2 mb-4">
                                                    {Array.isArray(project.technologies) && project.technologies.map((tech, i) => (
                                                        <span key={i} className="px-3 py-1 bg-gray-700/50 text-gray-300 rounded-full text-sm border border-gray-600/50">{tech}</span>
                                                    ))}
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex items-center pt-4 border-t border-gray-700/50 flex-col-reverse md:flex-row gap-4">
                                                ...
                                                <div className="flex items-center gap-3 flex-wrap">
                                                    <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => handleViewDetails(project)}
                                                        ...
                                                        <Eye size={18} /><span className="text-sm font-medium">Details</span>
                                                    </motion.button>
                                                    {project.githubUrl && (
                ...
                <style jsx>{`
                    ...
                    .custom-scrollbar::-webkit-scrollbar-track { background: rgba(75, 85, 99, 0.3); border-radius: 10px; }
